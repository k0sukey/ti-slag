...
function URLSession(__SLAG_PROPERTIES) {
...
URLSession.prototype.applyProperties = function (__SLAG_PROPERTIES) {
	for (var __SLAG_NAME in __SLAG_PROPERTIES) {
		this[__SLAG_NAME] = __SLAG_PROPERTIES[__SLAG_NAME];
	}
};
URLSession.prototype.createURLSessionBackgroundConfiguration = function (__SLAG_PROPERTY) {
	if (['ios'].indexOf(process.env.SLAG_PLATFORM) === -1) {
		throw new Error('This method is not support specified platform');
	}
	var URLSessionBackgroundConfiguration = require('./URLSession/URLSessionBackgroundConfiguration');
	return URLSessionBackgroundConfiguration(__SLAG_PROPERTY);
};
URLSession.prototype.createURLSession = function (__SLAG_PROPERTY) {
	if (['ios'].indexOf(process.env.SLAG_PLATFORM) === -1) {
		throw new Error('This method is not support specified platform');
	}
	var URLSession = require('./URLSession/URLSession');
	return URLSession(__SLAG_PROPERTY);
};

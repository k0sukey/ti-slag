...
function NdefMessage(__SLAG_PROPERTIES) {
	...
	if (['android'].indexOf(process.env.SLAG_PLATFORM) === -1) {
		...
		this.records = __SLAG_PROPERTIES.records || [];
	}
...
NdefMessage.prototype.setLifecycleContainer = function (__SLAG__PROPERTY) {
	if (['android'].indexOf(process.env.SLAG_PLATFORM) === -1) {
		throw new Error('This method is not support specified platform');
	}
...
NdefMessage.prototype.setRecords = function (__SLAG__PROPERTY) {
	if (['android'].indexOf(process.env.SLAG_PLATFORM) === -1) {
		throw new Error('This method is not support specified platform');
	}
	this.records = __SLAG__PROPERTY;
};

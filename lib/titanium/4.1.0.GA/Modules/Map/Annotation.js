...
function Annotation(__SLAG_PROPERTIES) {
	...
	if (__SLAG_PROPERTIES.apiName) {
		throw new Error('Modules.Map.Annotation.apiName is read only property');
	}
	...
	if ([
		...
		this.showInfoWindow = undefined;
	} else {
		this.showInfoWindow = __SLAG_PROPERTIES.showInfoWindow || true;
	}
...
Annotation.prototype.setShowInfoWindow = function (__SLAG__PROPERTY) {
	if ([
			'android',
			'ios'
		].indexOf(process.env.SLAG_PLATFORM) === -1) {
	...
	this.showInfoWindow = __SLAG__PROPERTY;
};

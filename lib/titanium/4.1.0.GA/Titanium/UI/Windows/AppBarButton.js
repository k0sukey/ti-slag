...
function AppBarButton(__SLAG_PROPERTIES) {
...
AppBarButton.prototype.applyProperties = function (__SLAG_PROPERTIES) {
	for (var __SLAG_NAME in __SLAG_PROPERTIES) {
...
AppBarButton.prototype.setIcon = function (__SLAG__PROPERTY) {
	if (['windowsphone'].indexOf(process.env.SLAG_PLATFORM) === -1) {
		throw new Error('This method is not support specified platform');
	}
...
AppBarButton.prototype.setTouchEnabled = function (__SLAG__PROPERTY) {
	if (['windowsphone'].indexOf(process.env.SLAG_PLATFORM) === -1) {
		throw new Error('This method is not support specified platform');
	}

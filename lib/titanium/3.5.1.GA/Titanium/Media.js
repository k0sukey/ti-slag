...
Media.prototype.createItem = function (__SLAG_PROPERTY) {
	if (['ios'].indexOf(process.env.SLAG_PLATFORM) === -1) {
		throw new Error('This method is not support specified platform');
	}
	var Item = require('./Media/Item');
	return Item(__SLAG_PROPERTY);
};
Media.prototype.createMusicPlayer = function (__SLAG_PROPERTY) {
	if (['ios'].indexOf(process.env.SLAG_PLATFORM) === -1) {
		throw new Error('This method is not support specified platform');
	}
	var MusicPlayer = require('./Media/MusicPlayer');
	return MusicPlayer(__SLAG_PROPERTY);
};
